...
async function createChatCompletion({ baseUrl, apiKey, models, body, fetchImpl = fetch, timeoutMs = 40000 }) {
  ...
  for (let index = 0; index < models.length; index += 1) {
    ...
    try {
      ...
      const wrapped = error?.name === 'AbortError' || controller.signal.aborted
        ? new TranslationProviderError('翻译服务请求超时', { code: 'TIMEOUT', cause: error })
        : new TranslationProviderError('无法连接翻译服务', { code: 'NETWORK', cause: error });
      lastError = wrapped;
      if (index < models.length - 1) {
        console.warn(`翻译模型 ${model} 请求${wrapped.providerCode === 'TIMEOUT' ? '超时' : '失败'}，自动切换到 ${models[index + 1]}`);
        continue;
      }
      throw wrapped;
    } finally {
