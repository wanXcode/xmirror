...
function extractXPostId(value = '') {
  const match = String(value).match(/\/(?:status|article)\/(\d+)(?:[/?#]|$)/i);
  return match ? match[1] : null;
}

function canonicalizeXPostUrl(value = '') {
  const input = String(value).trim();
  const postId = extractXPostId(input);
  if (!postId) throw new Error('无法提取推文ID');

  let parsed;
  try {
    parsed = new URL(input);
  } catch {
    throw new Error('无效的X链接');
  }
